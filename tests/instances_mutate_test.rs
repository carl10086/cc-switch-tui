...
async fn test_patch_instance_updates_model_and_apikey() {
    ...
    assert!(body.contains("\"modelId\":\"MiniMax-M2.7\""), "model not updated:\n{body}");
    assert!(body.contains("\"apiKey\":\"sk-updated\""), "apiKey not updated:\n{body}");
}
...
async fn test_delete_instance_returns_204() {
    ...
    assert!(!list_raw[body_start..].contains("cl-edit"), "instance should be deleted");
}
...
async fn test_duplicate_instance_adds_copy_suffix() {
    ...
    let raw = http_request(addr, "POST", "/api/instances/minimax-cl-edit/duplicate", None).await;
    assert!(raw.starts_with("HTTP/1.0 201"), "expected 201, got:\n{raw}");
    let body_start = raw.find("\r\n\r\n").map(|i| i + 4).unwrap_or(0);
    let body = &raw[body_start..];
    assert!(body.contains("\"alias\":\"cl-edit-copy\""), "expected alias=cl-edit-copy, got:\n{body}");
    assert!(body.contains("\"id\":\"minimax-cl-edit-copy\""), "wrong id, got:\n{body}");
}
...
async fn test_duplicate_collision_returns_409() {
    ...
    let _ = http_request(addr, "POST", "/api/instances/minimax-cl-edit/duplicate", None).await;
