...
async fn test_templates_returns_built_in_list() {
    ...
    // 至少包含 minimax + kimi
    assert!(
        body.contains("\"id\":\"minimax\""),
        "expected minimax template, got:\n{body}"
    );
    assert!(
        body.contains("\"id\":\"kimi\""),
        "expected kimi template, got:\n{body}"
    );
    // 字段名驼峰
    assert!(
        body.contains("\"displayName\""),
        "expected displayName field, got:\n{body}"
    );
    assert!(
        body.contains("\"availableModels\""),
        "expected availableModels field, got:\n{body}"
    );
    // minimax template 至少有一个 model
    assert!(
        body.contains("MiniMax-M3") || body.contains("M2.7"),
        "expected at least one model, got:\n{body}"
    );
    // B2 fix: models 数组 + 每项含 opencodeModelId
    assert!(
        body.contains("\"models\""),
        "expected models array field (B2), got:\n{body}"
    );
    assert!(
