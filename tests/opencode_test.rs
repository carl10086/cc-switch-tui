...
async fn test_get_opencode_config_returns_json() {
    ...
    assert!(body.contains("\"$schema\""), "expected $schema, got:\n{body}");
    assert!(body.contains("opencode.ai/config.json"), "expected opencode schema, got:\n{body}");
    assert!(body.contains("minimax-cn"), "expected provider id, got:\n{body}");
}
...
async fn test_apply_opencode_config_returns_path() {
    ...
    assert!(body.contains("cl-oc.json"), "expected file name, got:\n{body}");
}
