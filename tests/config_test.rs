...
async fn test_export_returns_versioned_json() {
    ...
    // 创建一条 instance
    let create_body = r#"{"templateId":"minimax","alias":"cl-exp","modelId":"MiniMax-M3","apiKey":"sk-x"}"#;
    ...
    assert!(raw.contains("attachment"), "expected Content-Disposition attachment");

    let body_start = raw.find("\r\n\r\n").map(|i| i + 4).unwrap_or(0);
    let body = &raw[body_start..];
    assert!(body.contains("\"version\": 1"), "expected version: 1, got:\n{body}");
    assert!(body.contains("\"exportedAt\""), "expected exportedAt, got:\n{body}");
    assert!(body.contains("cl-exp"), "expected cl-exp in export, got:\n{body}");
    // 关键：apiKey 不应出现在导出
    assert!(!body.contains("sk-x"), "apiKey must NOT be exported, got:\n{body}");
}
...
async fn test_import_creates_new_instances() {
    ...
    let body_start = raw.find("\r\n\r\n").map(|i| i + 4).unwrap_or(0);
    let body = &raw[body_start..];
    assert!(body.contains("\"created\":1"), "expected created:1, got:\n{body}");

    // 验证 list 包含
    let list = http_request(addr, "GET", "/api/instances", None).await;
    let list_body_start = list.find("\r\n\r\n").map(|i| i + 4).unwrap_or(0);
    assert!(list[list_body_start..].contains("cl-imp1"), "imported not in list");
}
...
async fn test_import_skips_existing_alias() {
    let addr = api::spawn_app().await;
    // 先创建
    let create_body = r#"{"templateId":"minimax","alias":"cl-dup","modelId":"MiniMax-M3","apiKey":"sk-x"}"#;
    http_request(addr, "POST", "/api/instances", Some(create_body)).await;
    ...
    let body_start = raw.find("\r\n\r\n").map(|i| i + 4).unwrap_or(0);
    let body = &raw[body_start..];
    assert!(body.contains("\"created\":0"), "expected created:0, got:\n{body}");
    assert!(body.contains("\"skipped\":1"), "expected skipped:1, got:\n{body}");
    assert!(body.contains("cl-dup"), "expected alias in skippedAliases, got:\n{body}");
}
...
async fn test_import_rejects_unknown_version() {
    ...
    let body_start = raw.find("\r\n\r\n").map(|i| i + 4).unwrap_or(0);
    let body = &raw[body_start..];
    assert!(body.contains("VALIDATION_ERROR"), "expected VALIDATION_ERROR, got:\n{body}");
}
