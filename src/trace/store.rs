...
impl TraceStore {
    ...
    pub fn append_record(
        ...
        self.conn
            .execute(
                "INSERT INTO records (session_id, record_index, turn, timestamp, direction, payload_json)
                 VALUES (?1, ?2, ?3, ?4, ?5, ?6)",
                rusqlite::params![
                    session_id,
                    next_index,
    ...
    pub fn list_sessions(
        ...
        let sessions = stmt
            .query_map([limit, offset], Self::row_to_session)
            .map_err(|e| AppError::Database(e.to_string()))?
            .collect::<Result<_, _>>()
            .map_err(|e| AppError::Database(e.to_string()))?;
    ...
    /// Get all records for a session, ordered by record_index.
    pub fn get_records(&self,
        ...
        let mut stmt = self
            .conn
            .prepare(
                "SELECT session_id, record_index, turn, timestamp, direction, payload_json
...
mod tests {
    ...
    fn test_append_record() {
        ...
        let records = store.get_records(&id).unwrap();
    ...
    fn test_list_sessions_pagination() {
        ...
        let page = store.list_sessions(1, 0).unwrap();
        assert_eq!(page.len(), 1);

        let page = store.list_sessions(10, 10).unwrap();
    ...
    fn test_delete_session_cascades() {
        ...
        store
            .append_record(&id, Some(1), TraceDirection::Request, "{}")
            .unwrap();

        store.delete_session(&id).unwrap();

        assert!(store.get_session(&id).unwrap().is_none());
        assert!(store.get_records(&id).unwrap().is_empty());
    }
